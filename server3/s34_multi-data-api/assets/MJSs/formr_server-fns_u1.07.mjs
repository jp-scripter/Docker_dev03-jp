...
# .(30608.01  6/08/23 RAM  9:13a|  Check for pValidArgs.action, and if MT 
# .(30608.02  6/08/23 RAM  8:00p|  Remove handler arg, aMethod, if necessary
# .(30608.04  6/08/23 RAM  8:00p|  Use aEntity, not aDatasetName 
# .(30608.05  6/08/23 RAM  8:34p|  Fix pReq.args if &action=
# .(30611.02  6/11/23 RAM  4:00p|  Use DB_UserId rather than DB_User in .env 
# .(30611.03  6/11/23 RAM  4:30p|  Pass pApp and pDB to new _Entity_Server
# .(30611.04  6/11/23 RAM  4:47p|  Use Sequelize
# .(30611.05  6/11/23 RAM  7:15a|  Should init() be an async function
# .(30613.01  6/13/23 RAM  9:15a|  Pass pApp, but not pDB_Config to init()
# .(30613.03  6/13/23 RAM  8:30a|  pDB is local or in this.DB
# .(30615.01  6/15/23 RAM  7:50a|  Write intiDB to set pDB separately
# .(30615.02  6/15/23 RAM  9:05a|  Get pEnv just for aDBSN in new version of formr_utility-fns_u1.07.mjs
# .(30616.02  6/16/23 RAM  8:10a|  Use bTraceR
# .(30619.01  6/19/23 RAM  8:10a|  Add aTestRoute
# .(30622.03  6/22/23 RAM  8:33a|  Set and return pDB.Err in getData and putData 
# .(30626.02  6/26/23 RAM  9:55a|  Trim aEntity after ? for JSON Object
# .(30626.03  6/26/23 RAM 11:30a|  Adjust sayMsg spacing 
# .(30626.04  6/26/23 RAM 11:55a|  Add CCase and aEntity to fncName 
# .(30627.02  6/27/23 RAM 10:45a|  Put aHandler in pRes for chkArgs and sndRecs 
# .(30627.03  6/27/23 RAM  3:10p|  Add bId option to chkSQL 
# .(30627.04  6/27/23 RAM  3:10p|  Redefine pArgs for INSERT and UPDATE
# .(30627.05  6/27/23 RAM  4:10p|  No sequenize.execute
# .(30626.02  6/28/23 RAM  7:53a|  Add aMsg in 

##SRCE =========================+===============================================+========================== #  ===============================  #
#*/
//  ----------------------------|  -------------------------------- ------------------- ------------------+ --------------------------

// import   mysql           from  'mysql2/promise';                                                         //#.(30611.04.1)
// import   Sequelize       from  'sequelize';                                                              //#.(30611.04.1 RAM Sequelize is imported in db.connect)
// import { dbConnect }     from  './assets/MJSs/db.connect_u4.01.mjs';                                     //#.(30611.04.1 RAM Use Sequelize)
   import { dbConnect }     from  './db.connect_u4.01.mjs';                                                 // .(30611.04.1 RAM Use Sequelize in same MJSs folder)

   import { inspect }       from  'util'                                                                    // .(30328.05.3 RAM Add)
   import   express         from  'express';
// import   bodyParser      from  'body-parser';                                                            // .(30424.09.3 RAM Use express.json)
   import   cssjson         from  'cssjson';                                                                // .(30402.02.5)
   import   cors            from  'cors';
// import   fs              from  'fs'

   import { getEnv_sync, __dirname, __appDir, traceR, aOS }  from  './formr_utility-fns_u1.07.mjs'          // .(30615.02.1).(30410.02.1).(30410.03.1 Add setAPI_URL).(30412.02.10).(30416.02.3).(30416.03.3)
   import { sayErr }                                         from  './formr_utility-fns_u1.07.mjs'          // .(30615.02.1).(30417.03.3)

       var  bQuiet     =  false    // it's global in this module
       var  bTraceR    =  false                                                                             // .(30616.02.3 RAM Use bTraceR)
       var  aEnv       = '../../.env'
...
//  ------  ---- ----- =  ------|  -------------------------------- ------------------- ------------------+
 ...
 async function  putData( pDB, aSQL, aEntity, pRes ) {                                                      // .(30407.03.1 Beg RAM Write function)
       var  mRecs         =  await putData_( pDB, aSQL, aEntity );
        if (mRecs[0] == 'error') {                                                                          // .(30622.03.4 RAM mRecs[0] == 'error' or pDB.Err != false)
                     if (pRes) {   sndErr(  pRes, mRecs[1] ); }                                             // .(30515.12.1 RAM What is this for???).(30622.03.4 RAM Send Error if Error) 
//          process.exit()
            return null                                                                                                                             // .(30511.02.2 RAM Return !mRecs to abort)
        } else {
    return  mRecs
            }
...
//  ------  ---- ----- =  ------|  -------------------------------- ------------------- ------------------+
 ...
 async function  putData_( pDB, aSQL, aEntity ) {                                                                                                   // .(30403.05.1 Beg RAM Write function).(30407.03.2)
       var  aRecords   = `${ aEntity ? aEntity.replace( /^\//, "" ) : '' } rec`                                                                     // .(30528.01.1 RAM Is needed).(30526.03.1)
...
//     var  mRecs      =  await pDB.execute( saySQL( aSQL, 'parse' ) );                                                                             //#.(30413.01.1 RAM Parse SQL).(30527.02.6 RAM Don't fmt for say, rather clean for execute).(30627.05.1)
       var  mRecs      =  await pDB.query(   saySQL( aSQL, 'parse' ) );                                                                             // .(30627.05.1 RAM No sequenize.execute).(30413.01.1 RAM Parse SQL).(30527.02.6 RAM Don't fmt for say, rather clean for execute)
...
//      if (aEntity) {  aRecords = `${aRecords}${ (mRecs[0].affectedRows == 1) ? '' : 's' } ${ nID ? `, ${nID},` : '' }` }                          //#.(30406.01.2).(30526.03.4)
        if (aEntity) {  aRecords = `${aRecords} for '${aEntity}'${ nID ? ` (${nID})` : '' }` }                                                      // .(30406.01.2).(30526.03.4)
...
//  return ["success",  ` ${ mRecs[0].affectedRows } ${ aRecords }, ${aAction}`, { affectedRows: mRecs[0].affectedRows , affectedId: nID } ]        //#.(30406.01.3).(30526.03.6)
    ...
    return [aSuccess,    `${ aRecords.substring(4) }, ${aAction}`, { affectedRows: nRecs , affectedId: nID } ]                                      // .(30406.01.3).(30526.03.6).(30527.03.2)
            ...
            pDB.Err = ["error", `Error:  ${pError.message}.  <br>  
                                          &nbsp; &nbsp; &nbsp; ${     saySQL( aSQL    ) }` ]
            }
    return  pDB.Err                                                                                                                                 // .(30622.03.3 RAM Set and return pDB.Err)
         }; // eof putData_                                                                                                                         // .(30403.05.1 End)
//  ------  ---- ----- =  ------|  -------------------------------- ------------------- ------------------+

 async function  getData( pDB, aSQL, aEntity, pRes ) {                                                                                              // .(30407.03.3 Beg RAM Write function)
       var  mRecs         =  await getData_( pDB, aSQL, aEntity );
        if (mRecs[0] == 'error') {                                                                                                                  // .(30622.03.4 RAM mRecs[0] == 'error' or pDB.Err != false)
...
//          }                                                                                                                                       //#.(30410.03.2 RAM ?? )
    ...
    return (mRecs[0] == 'nodata') ? [] : mRecs                                                                                                      // .(30407.03.4 RAM Return [] if nodata)
            }
         }; // eof getData                                                                                                                          // .(30407.03.3 End)
//  ------  ---- ----- =  ------|  -------------------------------- ------------------- ------------------+

 async function  getData_( pDB, aSQL, aEntity ) {                                                                                                   // .(30402.05.17).(30407.03.5)
       var  aRecords   = `${ aEntity ? aEntity.replace( /^\//, "" ) : '' } rec`                                                                     // .(30528.01.2 RAM Is needed).(30402.05.20).(30526.03.7 RAM Not needed).(30526.03.7 RAM it is for sayErr)
       try {
//     var [mRecs, mCols] =  await pDB.execute( saySQL( aSQL, 'parse' ) );                                                                          //#.(30413.01.2 RAM Parse SQL).(30527.02.7 RAM Don't fmt for say, rather clean for execute).(30611.04.2) 
       var [mRecs, mCols] =  await pDB.query(   saySQL( aSQL, 'parse' ) );                                                                          // .(30611.04.2 RAM Sequelize uses query).(30413.01.2 RAM Parse SQL).(30527.02.7 RAM Don't fmt for say, rather clean for execute)
//     var [mRecs, mCols] =  await pDB.execute( saySQL( aSQL ) );                                                                                   // .(30413.01.2 RAM Parse SQL).(30527.02.7 RAM Do fmt for sayMsg, don't parse which it didn't anyway)
       ...
       var  mColDefs      =  mCols.map( pRec => { return { Name: pRec.name, Type: pRec.type, Len: pRec.columnLength, Decs: pRec.decimals } } )
        if (mRecs.length == 0) {
       var  aMsg          =  `0 ${ aRecords.replace( /\?.*/, '' ) }s returned`                                                                      // .(30626.02.11 RAM Add aMsg)
//               sayErr(    ` *** NoData:  No ${ aRecords }s returned.\n${ saySQL( aSQL, 31 ) }` );                                                 //#.(30402.05.21).(30407.03.6).(30526.01.01).(30528.01.2)
                 sayErr(     saySQL( aSQL,  0, `  ** NoData:  ${aMsg}` ) );                                                                         // .(30626.02.12).(30528.01.2).(30402.05.21).(30407.03.6).(30526.01.01)
    return  [ "nodata",                             `NoData:  ${aMsg}<br>                                                                           // .(30407.03.7).(30526.01.02)
                                                 &nbsp; &nbsp; &nbsp; ${ saySQL( aSQL     ) }` ] // .replace( /"/g, '\\"' ) ];                      // .(30626.02.13).(30328.04.2).(30402.05.11 RAM To be sent as HTML)
     } else {
...
//     var  aRecords      = `${ ('* ' + nRecs).padStart(nRecs == 1 ? 5 : 4) } rec${ nRecs == 1 ? '' : 's' }`                                        // .(30528.01.2).(30526.03.16).(30526.03.9 RAM Added space for singular).(30526.03.9 RAM Don't, pad it with 4, 05)
       var  aRecords      = `${ ('* ' + nRecs).padStart(nRecs == 1 ? 6 : 4) } rec${ nRecs == 1 ? '' : 's' }`                                        // .(30528.01.2).(30526.03.16).(30526.03.9 RAM Added space for singular).(30526.03.9 RAM Don't, pad it with 4, 05)
//      if (aEntity) {  var aRecords = `${aRecords}${ (mRecs.length == 1) ? '' : 's' }`                                                             //#.(30402.05.18).(30526.03.10)
            aRecords      = (aEntity) ? `${aRecords} for '${aEntity}'` : aRecords                                                                   // .(30406.01.2).(30526.03.10)
...
//               sayMsg( `${ saySQL( aSQL ) }\n  ${ aRecords }, returned`.replace( /\n/g, '\n           ') );                                       //#.(30526.03.11).(30528.01.3)
    ...
    return  mRecs
                 ...
                 sayErr(   ` *** Error:  ${pError.message}.${aFill}${ saySQL( aSQL, -1 ) }\n` );                                                    // .(30529.01.4)
            pDB.Err = ["error", `Error:  ${pError.message}.  <br>  
                                          &nbsp; &nbsp; &nbsp; ${     saySQL( aSQL     ) }` ] // .replace( /"/g, '\\"' ) ];                         // .(30622.03.2).(30328.04.2).(30402.05.12 RAM To be sent as HTML)
    return  pDB.Err                                                                                                                                 // .(30622.03.3 RAM Set and return pDB.Err)
            }
         }; // eof getData
//  ------  ---- ----- =  ------|  -------------------------------- ------------------- ------------------+
...
//function  sndRecs( mRecs, aSQL, aEntity, pRes, aOnRouteFnc ) {                                            //#.(30608.04.1  RAM Changge aDataSetName to aEntity).(30407.03.8).(30627.02.21)
  function  sndRecs( mRecs, aSQL, aEntity, pRes ) {                                                         // .(30627.02.21).(30608.04.1  RAM Changge aDataSetName to aEntity).(30407.03.8)
              
       var  aRecords   = (aEntity ? aEntity.replace( /^\//, "" ) + " " : "" ) + 'records'                   // .(30526.03.12 RAM Keep??)
       var  aHandler   =  pRes.Handler                                                                      // .(30627.02.22) 

        if (String(mRecs[0]).match(/error/ )) {
            aJSON      = `{ "error": "${ mRecs[1].replace( /[ \n]+$/, '' ) }" }`                            // .(30526.03.13 RAM S.B double-quotes not tics: \')
        } else {
        if (mRecs.length > 0 || pRes.SndNoData) {
...
//     var  pRecs      =  mRecs; if (aEntity) { pRecs = {}; pRecs[aRecords]     = mRecs }                   //#.(30404.01.2)

            aEntity    =  aEntity.replace( /^\//, '' ).replace( /[\/\?].*/, '' )                            // .(30626.02.1 RAM Add delete after ?).(30608.04.2)
       var  pRecs      =  mRecs; if (aEntity) { pRecs = {}; pRecs[ aEntity ] = mRecs }                      // .(30608.04.3).(30404.01.2 RAM Use aEntity, not Records)
...
//                        sayErr( `${ saySQL( aSQL   ) }\n ** No ${aRecords} found` );                      //#.(30328.04.6).(30402.05.15 RAM Not the same as GetData).(30407.03.9 RAM Now done in getData)
       var  aJSON      =  JSON.stringify( {  "warning":   ` ** 0 ${aRecords.replace( /\?.*/, '' )} found`}) // .(30626.02.2).(30402.05.16)
         }  }
...
//               sayMsg( `Handler,     '${aOnRouteFnc ? aOnRouteFnc :'routeHandler'}', executed`);          //#.(30528.05.1 RAM Move it here).(30331.01.4).(30526.01.10).(30627.02.23)
                 sayMsg( `Handler,     '${aHandler}', executed`);                                           // .(30627.02.23).(30528.05.1 RAM Move it here).(30331.01.4).(30526.01.10)
...
//function  setRoute(  pApp, aMethod,  aRoute_, onRoute,  pValidArgs_, fmtSQL_ ) {                          //#.(30328.03.1 RAM Move to this script).(30328.03.5 RAM Need onRoute too).(30401.02.1)
  function  setRoute(  pApp, aMethod,  aRoute_, onRoute_, pValidArgs_, fmtSQL_ ) {                          // .(30401.02.1 RAM Rework it a bit)
      ...
      var { pValidArgs,   fmtSQL } = chkSQLargs( pValidArgs_, fmtSQL_ )                                       // .(30328.05.1 RAM Use chkArgs4SQL)
       ...
       var  bMethod    =  String(onRoute_).match( /\( *p*Req/i ) == null, onRoute                                               // .(30608.02.1 RAM Remove handler arg, aMethod, if necessary)
        if (bMethod)   {  onRoute  = async ( pReq, pRes ) => { onRoute_( aMethod, pReq, pRes, aRoute_, pValidArgs, fmtSQL ) }   // .(30608.02.2 RAM)
        } else         {  onRoute  = async ( pReq, pRes ) => { onRoute_(          pReq, pRes, aRoute_, pValidArgs, fmtSQL ) } } // .(30608.02.3 RAM)
//                   async function onRoute( pReq, pRes )    { onRoute_( aMethod, pReq, pRes, aRoute_, pValidArgs, fmtSQL ) }   //#.(30608.02.3).(30405.02.1 Use Original route)
//                                  async  ( pReq, pRes ) => { onRoute_( aMethod, pReq, pRes, aRoute,  pValidArgs, fmtSQL ) }   //#.(30405.02.1)
    switch (aMethod) {
...
//  ------  ---- ----- =  ------|  -------------------------------- ------------------- ------------------+
  ...
  function  chkSQL( fmtSQL, pArgs, bID ) {                                                                  // .(30627.03.1 RAM Add bId option).(30403.06.4 RAM Beg Write chkSQL )
        if (String( fmtSQL ).match( /INSERT/i ) ) {                                                         // .(30627.04.1 RAM Beg Redefine pArgs for INSERT and UPDATE)
       var  pArgs_ = pArgs, pArgs = { names: [], values: [] }                                               // .(30627.04.2 RAM Really pCols = pArgs)  
            Object.keys( pArgs_ ).forEach( aKey => { pArgs.names.push( CCase( aKey ) ); pArgs.values.push( `'${ pArgs_[ aKey ] }'`   ) } )
            }
        if (String( fmtSQL ).match( /UPDATE/i ) ) {                                                         
       var  pArgs_ = pArgs, mCols = [ ]                                                                     // .(30627.04.3 RAM Really mArgs = pArgs)  
            Object.keys( pArgs_ ).forEach( aKey => {       mCols.push( { name : CCase( aKey ), value :      `'${ pArgs_[ aKey ] }'` } ) } )
            pArgs.Cols = mCols }                                                                            // .(30627.04.4 End)  
     try {                                                                                                  // .(30511.02.3 RAM If fmtSQL bombs)
       var  aSQL       =  typeof( fmtSQL ) == 'string'  ? fmtSQL : fmtSQL( pArgs )
        ...
        if (pArgs.id && bID) {                                                                              // .(30627.03.2).(30511.02.5 RAM Add to id = pArgs.id, if ...)
        var nSort      =     aSQL.match( /ORDER BY|HAVING/i ); nSort = nSort ? nSort[0].index : aSQL.length // .(30627.03.3 RAM Check if ORDER BY exists)
        var aWhere     = `${ aSQL.match( /WHERE/i ) ? 'AND' : 'WHERE' } id = ${pArgs.id}`                   // .(30408.01.2 RAM Check if WHERE exists)
            aSQL       =     aSQL.match( /id *=/i ) ? aSQL                                                  // .(30403.06.2 RAM Kludge: Don't add if id = is present)
                       : `${ aSQL.substring( 0, nSort ) } ${aWhere} ${ aSQL.substring( nSort ) }`           // .(30627.03.4)
            }            
    return  aSQL
...
//  ------  ---- ----- =  ------|  -------------------------------- ------------------- ------------------+
  ...
  function  sndFile( pRes, aFile, aEntity, aOnRouteFnc ) {                                             // .(30403.04.1 Beg RAM Write function)
       try {
...
//  ------  ---- ----- =  ------|  -------------------------------- ------------------- ------------------+
  ...
  function  sndJSON( pRes, aJSON, aEntity ) {

            pRes.setHeader( 'Content-Type', 'application/json' );

        if (typeof(aJSON) == 'object') { aJSON = JSON.stringify( aJSON ) }                                  // .(30424.09.4)
            pRes.send(  aJSON )
            pRes.end();
        if (aJSON.match( /{ "error": /)) { return }
        if (aEntity) {
                 sayMsg( `JSON Object,  '${ aEntity.replace( /^\//, '' ).replace( /[\/\?].*/, '' ) }', sent` ) } // .(30626.02.3 RAM Add delete after ?).(30608.04.4).(30528.04.4 RAM No \n).(30526.01.15)

         }; // eof sndJSON
//  ------  ---- ----- =  ------|  -------------------------------- ------------------- ------------------+

  function  chkArgs( pReq, pRes, pValidArgs ) {
         if (Object.keys(  pReq.body).length == 0 && pReq.method != 'GET') {                                // .(30424.09.5 RAM Beg)
    ...
    return  null
       ...
       var  pArgs_ = { ...pReq.body, ...pReq.query }, mErrArgs = []                                         // .(30403.02.3 RAM pReq.query overrides pReq.body)
        ...
        if (pValidArgs.action) { return { action: pArgs_.action } }                                         // .(30608.01.1 RAM Todo: change if a valid action)

        if (Object.keys(pValidArgs).length == 0 && typeof(pArgs) != 'undefined') {                          // .(30608.01.2 RAM Better test)
...
//     var  mArgs      =  Object.keys( pReq.query ).map( aKey => { return [ aKey, pReq.query[aKey] ] } )    //#.(30403.02.5)
       var  pArgs = {};   Object.keys( pArgs_ ).map( aKey => { pArgs[ aKey.trim() ] = pArgs_[aKey].trim()}) // .(30403.07.1 RAM Trim pArgs).(30424.09.6 pArgs S.b. {}, not [] )

       var  mArgs      =  Object.keys( pArgs  ).map( aKey => { return [ aKey, pArgs[aKey]      ] } )        // .(30403.02.5)
        if (mArgs.length > 0) {
            mErrArgs   =  mArgs.filter( chkIf_BadArg )    
        } else {
...
//                        sayErr( `${aMsg}` ); sayMsg( 'Done', fncName(3) )                                 //#.(30529.01.6 RAM No \n, call sayMsg( 'Done', '') instead).(30402.05.7)
                          sayMsg( 'Abort', aMsg, fncName( 3, pRes.Handler ) )                               // .(30627.02.24).(30529.01.6 RAM Call sayMsg( 'Abort' ) instead).(30402.05.7)
    return  null
//          ----------------------------------
...
//  ------  ---- ----- =  ------|  -------------------------------- ------------------- ------------------+
  ...
  function  fmtArgs( pArgs, aRoute ) {                                                                      // .(30608.05.1 RAM Add optional aRoute) 
       var  mArgs =  Object.keys( pArgs ).map( aKey => { return [ aKey, pArgs[ aKey ].trim() ] } )          // .(30608.05.2 RAM Add trim())
       var  aArgs = (mArgs.length == 0) ? '' : '?' + mArgs.map( mArg => mArg.join('=') ).join( '&' )        // .(30608.05.3 RAM Change / to ?).(30608.05.4 RAM Add '&')    
        if (aRoute && aRoute.match( /action=/ )) { aArgs = aArgs.replace( /^\?/, '&' ) }                    // .(30608.05.4 RAM Change ? to &) 
    return  aArgs
...
//          pRes.setHeader( 'Content-Type', 'text/html' );                                                  // .(30424.09.7 RAM ??? Should it be json?)
//     var  aItems     =  mItems ? ( mItems.length > 0 ? `, '${ mItems.join(    ) }'` : "") : ""
       var  aItems     =  mItems ? ( mItems.length > 0 ? `, ${ mItems.join( ', ') }`  : "") : ""; aMsg = aMsg.replace( /[: ]$/, "" )  // .(30511.02.6 RAM ?? Trailing :)
        if (pRes.req.headers['accept'] == 'application/json') {                                             // .(30424.09.8)
                          sndJSON( pRes, { error: `${aMsg}: ${aItems.substring(2)}` } )                     // .(30424.09.9)
       } else {                                                                                             // .(30424.09.10)
...
//          aMsg       = `${ aMethod.toUpperCase().padEnd(4) } Route, '${aAPI_Host}${aRoute}, set`; break   //#.(30415.03.2).(30526.01.22 RAM Pad method).(30626.03.1)
            aMsg       = `${ aMethod.toUpperCase().padEnd(4) } Route,  '${aAPI_Host}${aRoute}, set`; break  // .(30626.03.1).(30415.03.2).(30526.01.22 RAM Pad method)
...
*/
            ...
            pReq.args  =  fmtArgs( pReq.query, aRoute )                                                     // .(30608.05.2).(30528.04.21 RAM For Excuted Route).(30414.03.3 RAM save for ending sayMsg)
//     var  aMsg       = `${ aMethod_.toUpperCase().padEnd(4) } Route, '${aAPI_Host}${aRoute}`              //#.(30528.04.22 RAM Use aMethod_).(30415.03.2).(30526.01.22 RAM Pad method).(30626.03.2)
       var  aMsg       = `${ aMethod_.toUpperCase().padEnd(4) } Route,  '${aAPI_Host}${aRoute}`             // .(30626.03.2).(30528.04.22 RAM Use aMethod_).(30415.03.2).(30526.01.22 RAM Pad method)
...
//  ------  ---- ----- =  ------|  -------------------------------- ------------------- ------------------+
  ...
  function  fncName( n, aEntity ) {                                                                         // .(30626.04.1).(30529.01.7 Beg RAM Write fncName)
       var  mStack = (new Error).stack.split("\n"); // console.log(mStack)
//     var  aRow   =  mStack[ n ? n : 2 ] || ''
       var  mFName = (mStack[ n ? n : 1] || '').match( /at (.+) \(/ )
        if (mFName && aEntity) {
       var  aEntity1   =  aEntity.replace( /^[_\/]/, '' ).replace( /[_\/\?].*/, '' )                        // .(30627.02.25 RAM Get aEntity just between _s).(30626.04.2)
//          mFName[1]  =  mFName[1].replace( /_Entity/, "_" + CCase( aEntity ) )                            //#.(30626.04.3).(30627.02.25)
            mFName[1]  =  mFName[1].replace( /_Entity/, "_" + aEntity1 )                                    // .(30627.02.25 RAM Don't CCase aEntity).(30626.04.3)
            }
    return  mFName ?  mFName[1] : ""
         };                                                                                                 // .(30529.01.7 End)
//  ------  ---- ----- =  ------|  -------------------------------- ------------------- ------------------+

  function  CCase( aStr ) {                                                                                 // .(30626.04.4 RAM Beg Write CCase) 
       var  mWords = aStr.split( '_' )
            mWords = mWords.map( aWord => aWord.substr( 0, 1 ).toUpperCase() + aWord.substr( 1 ) )
            return mWords.join( '' )
         };                                                                                                 // .(30626.04.4 End)   
//  ------  ---- ----- =  ------|  -------------------------------- ------------------- ------------------+

  function  initDB( aDBSN, pDB_ ) {                                                                         // .(30615.03.2 RAM).(30615.01.11 RAM Beg Write initDB) 
                                                                                                          
        if (pDB_ && pDB_.DBSN != aDBSN) { return pDB_ }                                                     // .(30615.03.12 RAM Check it here?).(30615.03.2 RAM Return current pDB is it is not different)

       var  aEnv_Dir   =  __appDir; // `${__dirname}/../..`                                                 // .(30322.03.1 Beg RAM Set different var).(30416.03.5)

//          process.env=        getEnv_sync(  `${aEnv_Dir}/.env`, aDBSN );                                  //#.(30615.02.2 RAM Get pEnv just for aDBSN).(30611.04.3)
//     var  pEnv       =   process.env                                                                      //#.(30611.04.4).(30222.01.2 RAM Get it myself).(30322.03.1 End).(30412.01.9 RAM no await)
       var  pEnv       =        getEnv_sync(  `${aEnv_Dir}/.env`, aDBSN );                                  // .(30615.02.2 RAM Get pEnv just for aDBSN).(30611.04.4)
//     var  pEnv       =  await getEnv(                   '.env', true  )    // Just the contents of .env   //#.(30611.04.4 RAM Maybe not)

//                        setVar2( 'DB_Host',     '45.32.219.12', 'host' )                                  //#Look in pDB_Config too
//                        setVar2( 'DB_UserId',   'nimdas'      , 'username' )                              //#.(30611.04.5 RAM Was DB_User)
...
//     var  pDB        =  mysql.createPool( pDB_Config )                                                    //#.(30611.04.6)
//        , pDB        =  dbConnect( pDB_Config ) // , 'checkDB' )                                          //#.(30611.04.6 Use dbConnect from FormR days) 

      var { Sequelize                                                                                       // .(30611.04.7 Use Sequelize)
          , pDB        
            }          =  dbConnect( pEnv       ) // , 'checkDB' )                                          // .(30611.04.8 Use dbConnect for Sequelize from FormR days) 
//          }          =  dbConnect( pDB_Config ) // , 'checkDB' )                                          //#.(30611.04.8) 

                          sayMsg( "" )
//                        sayMsg( 'USE', `Database: '${ pDB_Config.database }'` )                           //#.(30528.04.27 RAM Add Use too).(30323.03.1).(30526.01.24).(30611.04.9)
                          sayMsg( 'USE', `Database: '${ pDB.config.DBSN }'`     )                           // .(30611.04.9).(30528.04.27 RAM Add Use too).(30323.03.1).(30526.01.24)

// return { Sequelize                                                                                       //#.(30613.03.21 RAM return Sequelize)
//        , pDB }                                                                                           //#.(30613.03.22 RAM and pDB) 
   return   pDB                                                                                             // .(30613.03.22 RAM return just pDB)
        };  // eof initDB                                                                                   // .(30615.01.11 End) 
//  ------  ---- ----- =  ------|  -------------------------------- ------------------- ------------------+

//   async  function init( pApp, pDB_Config, bQuiet_, aAPI_ ) {                                             //#.(30611.05.3 RAM Add async again).(30412.02.1).(30410.03.3 RAM async)
//          function init( pApp, pDB_Config, bQuiet_, aAPI_ ) {                                             //#.(30611.05.3 RAM Nope, causes multi result to fail).(30412.02.1).(30410.03.3 RAM async).(30613.01.14)
            function init( pApp,             bQuiet_, aAPI_ ) {                                             // .(30613.01.14).(30611.05.3 RAM Nope, causes multi result to fail).(30412.02.1).(30410.03.3 RAM async)
...
//          process.env=  await getEnv( `${aEnv_Dir}/.env` )                                                // .(30222.01.2 RAM Get it myself).(30322.03.1 End).(30410.03.4 RAM await)
            process.env=        getEnv_sync(  `${aEnv_Dir}/.env` );                                         // .(30611.04.10)
       var  pEnv       =  process.env                                                                       // .(30611.04.11).(30222.01.2 RAM Get it myself).(30322.03.1 End).(30412.01.9 RAM no await)
//     var  pEnv       =  await getEnv( '.env', true )        // Just the contents of .env                  //#.(30611.04.11 RAM Maybe not)

            bQuiet     =  setVar1( 'Quiet', bQuiet_, true )   // Override value in .env                     // .(30628.01.2 RAM Switch?? return bQuiet over process.env[ 'Quiet' ])
            bQuiet     =  bQuiet ?  true :  false;            // console.log( `bQuiet:        ${bQuiet}`   );  process.exit()
/*                                                                                                          //#.(30615.01.11 RAM Beg Move to initDB) 
//                        setVar2( 'DB_Host',     '45.32.219.12', 'host' )                                  //#.(Look in pDB_Config too)
//                        setVar2( 'DB_UserId',   'nimdas'      , 'username' )                              //#.(30611.02.1 RAM Was DB_User)
//                        setVar2( 'DB_Password', 'FormR!1234'  , 'password' )
//                        setVar2( 'DB_Database', 'iodd'        , 'database' )                              //#.(30320.06.1 RAM Opps )
//                          
//          pDB  =  await mysql.createPool( pDB_Config ? pDB_Config : pDB_Config )
//     var  pDB        =  mysql.createPool( pDB_Config )                                                    //#.(30611.04.12)
//        , pDB        =  dbConnect( pDB_Config ) // , 'checkDB' )                                          //#.(30611.04.13 Use dbConnect from FormR days) 
//    var { Sequelize                                                                                       //#.(30611.04.14 Use Sequelize)
//        , pDB        
//          }          =  dbConnect( pEnv       ) // , 'checkDB' )                                          //#.(30611.04.15 Use dbConnect from FormR days) 
//          }          =  dbConnect( pDB_Config ) // , 'checkDB' )                                          //#.(30611.04.16) 
//
//                        console.log( "" )
//                        sayMsg( 'USE', `Database: '${ pDB_Config.database }'` )                           //#.(30528.04.27 RAM Add Use too).(30323.03.1).(30526.01.24).(30611.04.16)
//                        sayMsg( 'USE', `Database: '${ pDB.config.DBSN }'`     )                           //#.(30611.04.17).(30528.04.27 RAM Add Use too).(30323.03.1).(30526.01.24)
*/                                                                                                          //#.(30615.01.11 End) 
//          ---------  =  ----------------------------------------------------------

        pEnv.API_URL   =  pEnv.Remote_API ? pEnv.Remote_API : ''                                            // .(30616.02.1 RAM Allow Remote_API instead of API_URL)
        pEnv.API_URL   =  aAPI_ ?  aAPI_  : pEnv.API_URL;                                                   // .(30412.02.2)
//      pEnv.API_URL   =  aAPI_ ?  aAPI_  : pEnv.Remote_API_URL;                                            //#.(30616.02.1)
...
//     var  aAPI_URL   =        setAPI_URL( process.env )                                                   //#.(30410.04.2 Use it).(30410.04.9 RAM Add pEnv arg).(30412.02.3 RAM Can't be a promise)
       var  aAPI_URL   = `${pEnv.Host_Location == 'remote'                                                  //#.(30412.02.4)
                       ...
                       : `${pEnv.Local_Host}:${pEnv.Server_Port}` }`
                                                                                   bTraceR = true           // .(30616.02.2 RAM Use bTraceR) 
//          aAPI_Host  =  aAPI_ ? `/${aAPI_.replace( /^\//, '' )}` : aAPI_URL;      traceR( 'init[2]', `aAPI_Host:    '${aAPI_Host}'`    ); //#.(30410.03.7 RAM ??? ).(30412.02.5)
//          aAPI_Host  = (aOS != 'windows'              ) ? aAPI_URL : '';          traceR( 'init[2]', `aAPI_Host:    '${aAPI_Host}'`    ); //#.(30322.03.3 RAM Make it global)
            aAPI_Host  = (pEnv.Host_Location == 'remote') ? aAPI_URL : '';                                                                  // .(30412.02.5)

//          console.log( `aAPI: ${aAPI}, argv0: '${process.argv[1]}'`);

                          setVar1( 'Server_Port',  3000 );                          traceR( 'init[1]', `Server_Port:  '${process.env.Server_Port}'`, bTraceR ); // Look in process.env only
          aRemote_Host =  setVar1( 'Remote_Host', 'https:/iodd.com'  );             traceR( 'init[2]', `aRemote_Host: '${aRemote_Host}'`,            bTraceR ); //#.(30322.03.2 RAM I-Yi-Yai?).(30410.03.6 RAM Just set it)

//                        process.env.API_Host = aAPI_Host                                                                                                      // .(30412.02.6)
                          setVar1( 'API_Host',   aAPI_Host );                       traceR( 'init[3]', `aRemote_Host: '${aAPI_Host}'`,               bTraceR ); //#.(30322.03.2 RAM I-Yi-Yai?).(30410.03.6 RAM Just set it)

//          ---------  =  ----------------------------------------------------------

// return { pDB_: pDB, aAPI_Host_: aAPI_Host, bQuiet_: bQuiet }                                             //#.(30611.05.4 RAM Fails is async).(30322.03.4).(30615.01.8)
   return {            aAPI_Host_: aAPI_Host, bQuiet_: bQuiet }                                             // .(30615.01.8).(30611.05.4 RAM Fails is async).(30322.03.4

   function setVar2( aVar2, aValue, aVar1 ) {                                                               //  aVar2 in process.env, aVar1 in pDB_Config
            ...
            pDB_Config[aVar1] = typeof( pDB_Config[ aVar1 ] ) != 'undefined' ? pDB_Config[ aVar1 ]  : ( typeof( aValue ) != 'undefined' ? aValue : null )  // .(30322.06.1)
    return  pDB_Config[aVar1]
            }

   function setVar1( aVar,  aValue, bSw ) {                                                                 //  if bSwitch return aValue over process.env[ aVar ]
        if (bSw) { aValue  = typeof( aValue )   != 'undefined' ? aValue : process.env[ aVar ] || ''        //  if not     return process.env[ aVar ] over aValue 
//      if (bSw) { return    typeof( aValue )   != 'undefined' ? aValue : process.env[ aVar ] || '' }       //  if not     return process.env[ aVar ] over aValue 
//  return                   typeof( process.env[   aVar  ] ) != 'undefined' ? process.env[ aVar ] : ( typeof( aValue ) != 'undefined' ? aValue : null )    // .(30322.06.2)
        } else {   aValue  = typeof( process.env[   aVar  ] ) != 'undefined' ? process.env[ aVar ] : ( typeof( aValue ) != 'undefined' ? aValue : null )  } // .(30628.2.1 RAM Added { else {}).(30322.06.2)
    return  process.env[ aVar ] = aValue                                                                    // 
            } // eof setVar1  
//          ---------  =  ----------------------------------------------------------
         }; // eof init
//  ------  ---- ----- =  ------|  -------------------------------- ------------------- ------------------+
...
// function start( pApp, nPort, aAPI_Host ) {     // must be last
   function start( pApp, nPort, aTestRte ) {                                                                // .(30619.01.11 RAM Add aTestRoute).(30412.02.7 RAM Remove aAPI_Host)
...
//          aTestRte   =  aTestRte ? `/${aEntities}/${aTestRte}` : ''                                       //#.(30619.01.12)
//          aTestRte   = `${ aTestRte ? '/' : '' }${ aTestRte.replace( /^\//, '') }`                        // .(30619.01.12)
            aTestRte   =  aTestRte ? `${ aTestRte ? '/' : '' }${ aTestRte.replace( /^\//, '') }` : ''       // .(30619.01.13)
        ...
        if (aAPI_Host == '') {                                                                              // .(30214.03.11)
            console.log( `\n    Server is running at: http://localhost:${nPort}${aTestRte}` )               // .(30619.01.14).(30213.02.1 Change port from 3000 to 3002).(30213.02.5)
...
//     var  __filepath =  new Error().stack.match( /IODD-Server_u.+\.mjs/ )[0]                              //#.(30607.01.3).(30315.01.1 RAM get correct running server file)
//     var  __filepath =  new Error().stack.match( /C:.+api\/IODD-Server_u.+\.mjs/ )[0]                     //#.(30315.01.2)
//          __filepath = `${__dirname.replace( /assets\/mjs\//, "" )}/${__filepath }`                       //#.(30607.01.3).(30529.02.1 RAM Remove doubling of //s).(30315.01.2)
       var  __filename =  import.meta.url.replace( /^.+\//, "" )                                            // .(30607.01.3 RAM Add __filename)
       var  __filepath =  `${__appDir}/api/${__filename }`                                                  // .(30607.01.4 RAM May NOT always be in /api/)
...
//  ------  ---- ----- =  ------|  -------------------------------- ------------------- ------------------+
    ...
    export { putData, getData,   sndRecs,  fmtJSON,  sndJSON,  chkArgs,  fmtArgs,   fncName }               // .(30611.06.1 Add fncName).(30403.05.2 RAM Add putData)
    ...
    export { getIPAddr,  logIP,  fmtFld4SQL, initDB, CCase }                                                // .(30626.04.5).(30615.01.12).(30526.02.3).(30527.02.15)
