...
//     ---  ------------------  =  -------------------------------- -------------------
       ...
       var  pServer    =  new _Entity_Server( pApp  )                                   // .(30615.03.12).(30613.01.5).(30611.03.1 RAM Defined this.methods in "class")
...
//========  =============================================================================================== #  ===============================  #
  ...
  function  _Entity_Server( pApp_ ) {                                                   // .(30615.03.10)
       ...
       var  aEntities           = "countries"                                           // .(30613.01.6)
       var  aEntity             = "Country"                                             // .(30613.01.7)
...
this._Entity_testRoute  = function( ) {                                                 // .(30619.01.4
...
//     ---  ------------------  =  -------------------------------- -------------------
    ...
    return "SELECT  count(*) as CNT FROM country"                                       // .(30622.02.1 RAM Always use CNT, not Cnt)
                }
//     ---  ------------------  =  -------------------------------- -------------------
...
//     ---  ------------------  =  -------------------------------- -------------------
  ...
  function  fmtSelectSQL( pArgs ) {
       ...
       var  aSQL       = `
          SELECT  country.*
            FROM  country
           WHERE  country.code Like '${ pArgs.id.toUpperCase() }%'
        ORDER BY  Name `
            }
  ...
  function  fmtUpdateSQL( pArgs ) {                                                             // .(30627.04.6 RAM Add Cols to pArgs in chkSQL)
       ...
       var  aSQL       = `
          UPDATE  country
           ...
           WHERE  country.code = '${ pArgs.code.toUpperCase() }' `
...
//     ---  ------------------  =  -------------------------------- -------------------
  ...
  function  fmtDeleteSQL( pArgs ) {
       ...
       var  aSQL       = `
          DELETE
            FROM  country
           WHERE  country.code = '${ pArgs.id.toUpperCase() }' `

    return  aSQL
            };  // eof fmtDeleteSQL
//     ---  ------------------  =  -------------------------------- ------------------- ------------------+

//          setRoute( pApp, 'get', '/countries', Countries_getRoute_Handler )                   //#.(30622.04.3)
            setRoute( pApp, aMethod,  aRoute, _Entity_getRoutes_Handler )                       // .(30622.04.3 RAM Use aRoute)
...
//     ---  ------------------  =  -------------------------------- -------------------
       ...
       var  pUpdateArgs         = { code:   /[a-zA-Z0-9]{3}/            // , 'required'
                                  , name:   /[a-zA-Z][a-zA-Z0-9_]+/
                                    }
//     ---  ------------------  =  -------------------------------- -------------------
...
//          setRoute( pApp, 'post', '/countries', Countries_postRoute_Handler )                 //#.(30622.04.7)
            setRoute( pApp, aMethod,  aRoute, _Entity_postRoutes_Handler )                      // .(30622.04.7 RAM Use aRoute)
